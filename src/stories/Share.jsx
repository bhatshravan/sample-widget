...
export const Share =({data,dataPerson,dataGroup}) => {
  const clipboard = useClipboard({ timeout: 500 });

  const [openType, setopenType] = useState("none");
  const [shareList, setshareList] = useState(data);
  const [personList, setpersonList] = useState(dataPerson.slice(0, 2));
  const [groupList, setgroupList] = useState(dataGroup.slice(0, 2));
  const [currentSelected, setcurrentSelected] = useState(null);
  const [currentAccess, setcurrentAccess] = useState("Full Access");
  const [currentInp, setCurrentInp] = useState(null);
  const handleKeyDown = (event) => {
    if (event.key === 'Enter'  && openType==="addUser") {
  ...
  useEffect(() => {
    if (currentInp && currentInp.length > 0) {
      ...
      setgroupList(dataGroup.slice(0, 2));
      setpersonList(dataPerson.slice(0, 2));
    }
  }, [currentInp]);
